...
import codecs
import json
from .. import globalConfigPara as gcp
...
def get_event_str(events_dict, event_dict):
    ...
    for event in events_dict:
        ...
        if PETRglobals.WriteNer:
            ...
            event_str += '\tlocation\t{}\n'.format(locationText)
            if locationText:
                event_outer_dict.update({"location": locationText})
            else:
                event_outer_dict.update({"location": "未提取出地点"})
        ...
        if PETRglobals.WriteSentenceTime:
            ...
            event_outer_dict.update({"sentenceTime": timeStamp})
